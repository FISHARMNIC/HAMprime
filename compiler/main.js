...
function compileLine(line) {
    ...
    for (; wordNum >= 0; wordNum--) {
        ...
        if (word == "DEFINED") {
            ...
            var sz = type.templatePtr == undefined ? type.size : asm.calcSizeOfFormatRef(type.templatePtr)
                ...
                if (!type.special) { // simple type
                    ...
                    } else if (word == "initializer") // if constructor
                    {
                        ...
                        if (lbl == -1) // IF IS INIT.
                        {
                            ...
                            var totSize = asm.calcSizeOfFormat(word)
        ...
        else if (objectIncludes(userVariables, word)) // word = global variable
        {
            ...
            if (requestBracketStack.type == "function") {
                ...
                var totSize = asm.calcSizeOfFormat(data.name);
            ...
            if (data.type == "function") {
                ...
                debugPrint("NEW FMT", _name, defines.types[_name])
            }
